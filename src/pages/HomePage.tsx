...
import { TrendingUp, Package, Tag, AlertCircle, Phone, MessageCircle, Table } from 'lucide-react';
...
export function HomePage() {
  ...
  const stats = [
    ...
    { label: t.dash_total_val, value: `${totalValue.toLocaleString()} SAR`, icon: TrendingUp, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    ...
    <div className="space-y-4">
      <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 text-center">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex flex-col items-center justify-center gap-1">
            <span className="text-slate-400 font-bold text-[9px] uppercase tracking-tighter">{t.current_login}</span>
            <span className="font-mono font-black text-emerald-600 text-sm">{loginTime}</span>
          </div>
          <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex flex-col items-center justify-center gap-1">
            <span className="text-slate-400 font-bold text-[9px] uppercase tracking-tighter">{t.last_visit}</span>
            <span className="font-mono font-black text-red-600 text-sm">{lastVisit}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3">
        {stats.map((stat, i) => (
          <motion.div 
            ...
            className="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex items-center gap-4 active:scale-[0.98] transition-transform"
          >
            ...
            <div>
              ...
              <p className="text-base font-black dark:text-white leading-tight">{stat.value}</p>
            </div>
          </motion.div>
        ))}
      </div>
